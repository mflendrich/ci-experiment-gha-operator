...
# If RECREATE is set to "true", delete any existing KinD cluster.
...
if ! kind get clusters | grep -q "^${CLUSTER_NAME}$"; then
    kind create cluster --name="${CLUSTER_NAME}" --config="${repo_root}/hack/kind/cluster-config.yaml" --retain
else
